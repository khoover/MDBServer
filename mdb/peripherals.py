                ...
                try:
                    ...
                    if poll_reset:
                        ...
                        if '06' not in response_statuses:
                            logger.warning("Did not get JUST RESET in the "
                                           "first poll after resetting, trying"
                                           "again.")
                            send_reset = True
                            # poll_reset = True
                            continue
                    ...
                    logger.info('Getting bill validator setup information.')
                    ...
                    logger.debug(f'Bill validator level: {self.feature_level}')
                    ...
                    logger.info('Getting bill validator expansion '
                                'information.')
                    self.expansion_data = \
                        await self.sendread_nolock_until_data_or_nack(
                            expansion_command)
                    logger.info('Got expansion data for bill validator: '
                                f'{self.expansion_data}')

                    logger.info('Getting stacked bill count.')
                    stacker_count = \
                        await self.sendread_nolock_until_data_or_nack(
                    ...
                    logger.warning("Bill validator timed out while resetting, "
                                   f"command was '{e.command}'.")
                    send_reset = True
                    poll_reset = True
                    continue
